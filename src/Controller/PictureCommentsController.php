...
class PictureCommentsController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->user_id == $this->request->data['user_id']) {
            ...
            if ($this->request->is('post')) {
                $pictureComment = $this->PictureComments->patchEntity($pictureComment,
                    $this->request->data);
                if ($this->PictureComments->save($pictureComment)) {
                    ...
                    if ($this->user_id != $query->user_id) {
                        ...
                        $this->Notifications->add($query->user_id,
                            'huntcomment', $query->mark->hunt->id,
                            $query->mark->name);
                    }
    ...
    public function edit($id = null)
    {
        ...
        $pictures = $this->PictureComments->Pictures->find('list',
            ['limit' => 200]);
